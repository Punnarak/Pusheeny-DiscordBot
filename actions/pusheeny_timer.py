...
class TimerCommand(commands.Cog):
    ...
    @commands.command(
        name="timer",
        help="ตั้งเวลา เช่น !timer 1 หรือ !timer 0.20 ข้อความ")
    async def timer(self, ctx, time_arg: str, *, custom_message: str = None):
        try:
            ...
            await ctx.send(
                "❌ รูปแบบเวลาผิดนะ ใช้แบบ `!timer 1` หรือ `!timer 0.20 ข้อความ`"
            )
            return

        # ส่งข้อความเริ่มต้น
        countdown_msg = await ctx.send(
            f"⏳ เริ่มนับถอยหลัง {total_seconds} วินาที...")

        # แสดง countdown ทุก 5 วิ
        for remaining in range(total_seconds, 0, -1):
            if remaining % 5 == 0 or remaining <= 5:
                await countdown_msg.edit(
                    content=f"⏳ เหลือเวลา {remaining} วินาที...")
